...
def derive_individual_slopes(
    ...
    for modality, brain_features in features_of_interest.items():
        logging.info(
            "Processing modality: %s (%d features)", modality, len(brain_features)
        )
        ...
        for feature in brain_features[:2]:
            ...
            try:
                ...
                # Construct formula
                formula = f"{feature} ~ {' + '.join(feature_fixed_effects)} + {random_effects}"  # noqa: E501
                ...
                if converged:
                    logging.info(
                        "Model converged for feature: %s (modality: %s)",
                        feature,
                        modality,
                    )

                    # Extract subject slopes
                    # (this returns a polars DataFrame from pymer4)
